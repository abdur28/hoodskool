...
// Helper function to create multi-currency prices
// Conversion rate: 1 USD = ~100 RUB (adjust as needed)
const USD_TO_RUB = 80;
...
function createPrices(usdPrice, usdComparePrice = null) {
  ...
  prices.push({
    ...
    discountPercent: usdDiscount, // Same discount percentage
  });
  
  return prices;
}

// Helper to create Color objects
function createColors(colorNames) {
  const colorMap = {
    'Black': '#000000',
    'White': '#FFFFFF',
    'Cream': '#F5F5DC',
...
const products = [
  ...
  // T-SHIRTS
  {
    ...
    colors: createColors(['Black', 'White']),
    sizes: ['S', 'M', 'L', 'XL'],
  ...
  // PANTS
  {
    ...
    colors: createColors(['Black', 'Olive']),
  ...
  // ACCESSORIES
  {
    ...
    colors: createColors(['Black', 'Navy', 'Beige']),
    sizes: ['One Size'],
  ...
  },
  {
    ...
    colors: createColors(['Black', 'White', 'Navy']),
    sizes: ['One Size'],
    materials: ['100% Cotton'],
    isBestseller: true,
    viewCount: 389,
  ...
  // BESPOKE
  {
    ...
    itemType: 'Suits',
    categoryPath: 'Clothings > Bespoke/Tailoring > Suits',
    images: [
      {
        id: 'img-9',
    ...
    colors: createColors(['Navy', 'Charcoal', 'Black']),
    sizes: ['36', '38', '40', '42'],
...
async function seedProducts() {
  console.log('🌱 Starting multi-currency product seeding...\n');
  console.log(`💱 Using conversion rate: 1 USD = ${USD_TO_RUB} RUB\n`);
  ...
  for (const productData of products) {
    ...
    console.log(`   💵 USD: $${product.prices[0].price} | RUB: ₽${product.prices[1].price}`);
    if (product.prices[0].compareAtPrice > 0) {
      console.log(`   💰 Discount: ${product.prices[0].discountPercent}% off`);
    }
  }

  await batch.commit();

  console.log(`\n✨ Successfully seeded ${count} products with multi-currency support!`);
